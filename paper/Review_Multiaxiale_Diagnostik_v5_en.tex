...
\begin{abstract}
\noindent The abolition of the multiaxial diagnostic system in DSM-5 (2013) left a structural gap in psychiatric diagnostics. This paper presents a 6-axis model for computer-assisted multiaxial diagnosis that unifies categorical classification according to DSM-5-TR and ICD-11 with the functional assessment of the ICF in a decision support system. The model comprises six axes: Axis~I (Mental Health Profiles with temporal diagnostics and exploratory coverage analysis), Axis~II (dimensional personality assessment via PID-5 and biographical contextualization), Axis~III (Medical Synopsis in symmetric parallel structure to Axis~I), Axis~IV (ICF integration, WHODAS~2.0, Cultural Formulation), Axis~V (operationalized case formulation in a 3P/4P model), and Axis~VI (evidence matrix, CAVE alerts, longitudinal symptom tracking). Three central design proposals are highlighted: (1)~the Diagnostic Coverage Index as a hypothesis-generating audit index for links between recorded symptom concepts and the current formulation, (2)~the symmetric Axis~I/III architecture for interprofessional documentation, and (3)~the operationalized connection between classification and individualized case formulation. The six-step gatekeeper logic follows the differential-diagnosis sequence described by First (2024). A Python reference implementation as a hierarchical state machine is publicly available. This is a methods and design paper; empirical validation is still pending, and the DCI is not a validated measure of diagnostic completeness, accuracy, or patient benefit.
...
\textbf{PRO/CONTRA evidence evaluation:} Each diagnosis receives a structured evidence evaluation with explicit documentation of findings supporting (PRO) and opposing (CONTRA) the diagnosis, together with a numerical confidence entry (0--100\%). Until prospectively calibrated, this entry is an explicitly subjective rating, not a probability or an estimate of diagnostic accuracy. Its purpose is to expose the clinician's reasoning and uncertainty, not to certify the diagnosis.
...
For the German context, the \textbf{GdB integration} (Grad der Behinderung, degree of disability) is essential. The GdB uses a 20--100 scale ($\geq 50$ = severe disability) and is assessed according to the Versorgungsmedizinische Grunds\"atze (VMG). International implementations require the applicable jurisdiction-specific individual disability procedure. Population-level burden measures such as disability-adjusted life years (DALYs) are not individual disability assessments and must not be used to infer an individual patient's impairment; case-level functioning remains documented through history and appropriately licensed, population-appropriate ICF-/WHODAS-based measures.
...
\begin{table}[H]
\centering
\caption{Exemplary interprofessional axis assignment (non-mandatory)}
...
The present system prefers instruments that can be accessed without a purchase fee; the proprietary CBCL/YSR complex \citep{Achenbach2001} is acknowledged as a reference standard. No-fee access does not itself establish permission to reproduce item text, translate, redistribute, administer digitally, or implement scoring. These rights must be verified instrument by instrument and version by version before inclusion. Table~\ref{tab:instrumente_ca} lists candidate child/adolescent (C/A) instruments in parallel to the adult matrix (Table~\ref{tab:instruments}).
...
Let $S = \{s_1, \ldots, s_n\}$ be a set of normalized symptom concepts and $D = \{d_1, \ldots, d_m\}$ the set of established diagnoses. Before scoring, observations must be entered into a symptom-concept ledger with concept ID, source instrument and item, informant, time window, and assessment occasion. Repeated or semantically overlapping observations are clustered and contribute at most once to the denominator; additional observations remain corroborating provenance rather than independent symptoms. Missing or unavailable information is reported separately and is not scored as absent or unexplained. For each $s_i$, define $c(s_i) = \max_{d_j \in D} w(s_i, d_j)$, where $w(s_i, d_j) \in [0, 1]$ is a heuristic criterion-link weight. The exploratory \textbf{Diagnostic Coverage Index (DCI)} is $C_{\text{total}} = \frac{1}{n} \sum_{i=1}^{n} c(s_i)$. It is a hypothesis-generating audit index, not a measure of diagnostic completeness, accuracy, validity, or patient benefit. The weighting and max aggregation require prospective calibration and comparison with prespecified alternatives.

\subsection{Threshold Rationale and Sensitivity}

The proposed thresholds (85\%/60\%) are preliminary display categories derived from constructually unrelated analogies, not clinical decision thresholds. They require prospective calibration in at least three clinical cohorts. Because $C_{\text{total}}$ is an arithmetic mean, an isolated weight perturbation shifts it by $\Delta C = \Delta w/n$; this algebra does not establish robustness to duplicated concepts, correlated instrument items, systematic weight error, target leakage, or changes in the symptom ontology. Until those failure modes pass negative controls and cluster-/instrument-level resampling, thresholds must not guide clinical action and results should be shown with a sensitivity range rather than a precision percentage.
...
The exploratory index may flag candidate information gaps for review; it must not be presented as quantitative proof of the completeness of diagnostic work. A score is held back unless provenance and deduplication checks pass and raw versus deduplicated results, duplicate-item controls, relabelled-axis controls, and target-leakage controls are documented. Related approaches exist in Bayesian diagnostic systems, but the present proposal remains an unvalidated audit design.
...
Earlier versions shipped a hand-curated seed catalogue of psychiatric codes inside the repository. That is adequate for demonstrating an axis model, but it is a common failure mode of clinical coding software: a private copy of a classification silently diverges from the maintained one, and the divergence is invisible precisely where it matters---at the point of coding. The implementation therefore no longer maintains a local copy. It \textbf{binds external catalogue modules} behind a single provider interface (Table~\ref{tab:catalog-bindings}), and the bundled seed catalogue is demoted to an explicit offline fallback.
...
ICD-10-CM (US) & \texttt{simple-icd-10\allowbreak-cm} & Public-domain CDC/NCHS & Offline \\
...
\textbf{Verification is tri-state.} A lookup returns \texttt{verified}, \texttt{not\_found}, or \texttt{unavailable}. The third state exists because the distinction between \emph{``the catalogue says this code does not exist''} and \emph{``we could not ask the catalogue''} is clinically consequential. Collapsing \texttt{unavailable} into \texttt{not\_found} would present an unverifiable code as refuted; collapsing it into \texttt{verified} would be more problematic still. The system does neither.
...
CAVE: Hypothyroidism may contribute to fatigue---review thyroid status and treatment in parallel; do not delay otherwise indicated or urgent depression treatment solely pending thyroid testing. \\
Symptom course: Loss of drive since November 2025, progressive; back pain since 2024, fluctuating.
\end{description}

\noindent This vignette illustrates the proposed workflow, not clinical validity. Its illustrative 78\% DCI has no calibrated clinical interpretation and is shown only to demonstrate the calculation. The CAVE alert flags a possible thyroid contribution for parallel clinical review while leaving treatment timing to individualized clinical judgment, and the condition model connects biographical vulnerability with the current trigger.
...
\item[Responsibility and product liability] Responsibility may be distributed among clinicians, operators, institutions, vendors, and developers. Open-source licensing and warranty disclaimers do not by themselves determine or eliminate statutory liability. Applicable duties, defect, damage, and causation require jurisdiction-specific legal assessment.
...
\textbf{Procedure:} Phase~2 is a feasibility and reliability-estimation pilot, not initial system validation. Raters are randomized to sequence and assess two matched, non-overlapping vignette sets, one per method; the sets are crossed between sequences. Period and sequence effects are reported. The expert panel supplies a blinded consensus reference with recorded disagreement and uncertainty, not a gold standard.
...
\begin{itemize}[leftmargin=*, noitemsep]
    ...
    \item \textit{Agreement with the consensus reference:} Descriptive agreement and exact confidence intervals. Diagnosis-specific sensitivity and specificity require a later, separately powered sample with adequate cases per diagnosis.
    \item \textit{Exploratory DCI signal:} Blinded adjudication of actionable information gaps and false-alert rate. This pilot does not test a confirmatory patient-benefit hypothesis.
...
\textbf{Sample-size status:} The proposed $N=10$ raters and 20 vignettes are a feasibility sample for workflow, recruitment, missingness, variance components, and interval estimation. Before recruitment, one primary estimand and analysis model must be preregistered and the target sample chosen by precision calculation or simulation. The pilot is not powered for diagnosis-specific accuracy, patient benefit, or superiority claims.
...
\textbf{HL7 FHIR (R4):} The following are candidate mappings for a future implementation guide, not demonstrated interoperability. Diagnoses could use \nolinkurl{Condition}; questionnaire answers \nolinkurl{QuestionnaireResponse}; scored functioning and assessment results \nolinkurl{Observation}; a clinician's synthesis may use \nolinkurl{ClinicalImpression}; care planning may use \nolinkurl{CarePlan}; and source documents may use \nolinkurl{DocumentReference}. Final resources and profiles depend on workflow, terminology bindings, provenance, consent, jurisdiction, and conformance testing.
...
The proposed 6-axis system responds to a structural gap in psychiatric diagnostics that has persisted since the abolition of the DSM-IV multiaxial system. Its current contribution is an evaluable design proposal for structured biopsychosocial documentation. Whether the symmetric architecture improves interprofessional work, the DCI identifies useful information gaps, or CAVE alerts improve safety remains to be tested. PRO/CONTRA entries and uncalibrated confidence ratings are traceability aids, not evidence of diagnostic accuracy or clinical benefit.

The model's architecture is consistent with the institutional direction of development (cf.\ Section~\ref{sec:introduction}). The convergence with the explicit call by \citet{ErlichFirst2025} for a return to structured axial assessment underscores the timeliness of this contribution. At the same time, the model extends beyond institutional revisions, particularly through the coverage analysis and the operationalized case formulation (Axis~V) that bridges classification and treatment planning.
