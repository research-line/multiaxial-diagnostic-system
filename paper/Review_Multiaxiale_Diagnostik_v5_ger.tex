...
\begin{abstract}
\noindent Die Abschaffung des multiaxialen Diagnosesystems im DSM-5 (2013) hinterlie\ss{} eine strukturelle L\"ucke in der psychiatrischen Diagnostik. Die vorliegende Arbeit pr\"asentiert ein 6-Achsen-Modell zur computergest\"utzten multiaxialen Diagnostik. Drei Designvorschl\"age werden hervorgehoben: (1)~der Diagnostic Coverage Index als hypothesengenerierender Audit-Index f\"ur Verkn\"upfungen zwischen dokumentierten Symptomkonzepten und aktueller Formulierung, (2)~die symmetrische Achse-I/III-Architektur f\"ur interprofessionelle Dokumentation und (3)~die operationalisierte Verbindung von Klassifikation und Fallformulierung. Dies ist ein Methoden- und Designpapier; die empirische Validierung steht aus. Der DCI ist kein validiertes Ma\ss{} f\"ur diagnostische Vollst\"andigkeit, Genauigkeit oder Patientennutzen.
...
\textbf{PRO/CONTRA-Evidenzbewertung:} Jede Diagnose erh\"alt eine strukturierte Evidenzbewertung sowie einen numerischen Konfidenzeintrag (0--100\%). Bis zu einer prospektiven Kalibrierung ist dieser Eintrag eine ausdr\"ucklich subjektive Einsch\"atzung, keine Wahrscheinlichkeit oder Sch\"atzung diagnostischer Genauigkeit. Er macht Begr\"undung und Unsicherheit sichtbar, zertifiziert aber keine Diagnose.
...
F\"ur den deutschen Kontext ist die \textbf{GdB-Integration} (Grad der Behinderung) essentiell. Der GdB verwendet eine 20--100-Skala ($\geq 50$ = Schwerbehinderung) und wird nach den Versorgungsmedizinischen Grunds\"atzen (VMG) bewertet. Internationale Implementierungen ben\"otigen das jeweilige individuelle, jurisdiktionsspezifische Verfahren. Populationsma\ss{}e wie DALYs sind keine individuelle Behinderungsbewertung und d\"urfen nicht zur Ableitung der Beeintr\"achtigung einer einzelnen Person verwendet werden.
...
Das vorliegende System bevorzugt Instrumente ohne Kaufgeb\"uhr. Dies belegt jedoch keine Erlaubnis zur Reproduktion, \"Ubersetzung, Weitergabe, digitalen Durchf\"uhrung oder Implementierung der Auswertung. Diese Rechte sind vor Aufnahme instrument- und versionsbezogen zu pr\"ufen. Tabelle~\ref{tab:instrumente_kj} listet Kandidaten parallel zur Erwachsenenmatrix.
...
Sei $S = \{s_1, \ldots, s_n\}$ eine Menge normalisierter Symptomkonzepte und $D = \{d_1, \ldots, d_m\}$ die Menge etablierter Diagnosen. Vor der Berechnung werden Beobachtungen in einem Symptomkonzept-Ledger mit Konzept-ID, Quellinstrument/-item, Informant, Zeitfenster und Erhebungszeitpunkt normalisiert. Wiederholte oder semantisch \"uberlappende Beobachtungen werden geclustert und gehen h\"ochstens einmal in den Nenner ein; weitere Beobachtungen bleiben als best\"atigende Provenienz erhalten. Fehlende Angaben werden separat ausgewiesen. Der explorative \textbf{Diagnostic Coverage Index (DCI)} ist $C_{\text{total}} = \frac{1}{n} \sum_{i=1}^{n}\max_{d_j\in D}w(s_i,d_j)$. Er ist ein hypothesengenerierender Audit-Index, kein Ma\ss{} f\"ur diagnostische Vollst\"andigkeit, Genauigkeit, Validit\"at oder Patientennutzen.
...
Die vorgeschlagenen Schwellenwerte (85\%/60\%) sind lediglich vorl\"aufige Anzeigekategorien aus konstruktfremden Analogien, keine klinischen Entscheidungsschwellen. Sie ben\"otigen prospektive Kalibrierung. Die Beziehung $\Delta C=\Delta w/n$ beschreibt nur eine isolierte Gewichts\"anderung; sie belegt keine Robustheit gegen duplizierte Konzepte, korrelierte Instrumentitems, systematische Gewichtsfehler, Target Leakage oder Ontologie\"anderungen. Bis diese Fehlerarten Negativkontrollen und Cluster-/Instrument-Resampling bestanden haben, d\"urfen Schwellen keine klinische Handlung steuern; Ergebnisse sind mit Sensitivit\"atsbereich statt als Pr\"azisionsprozent auszugeben.
...
Der explorative Index kann Kandidaten f\"ur Informationsl\"ucken markieren; er darf nicht als quantitativer Nachweis diagnostischer Vollst\"andigkeit dargestellt werden. Ein Wert wird nur angezeigt, wenn Provenienz- und Dublettenpr\"ufung bestanden sind und Roh-/deduplizierter Wert, Duplicate-Item-, Relabelled-Axis- und Target-Leakage-Negativkontrollen dokumentiert sind.
...
CAVE: Hypothyreose kann zur Fatigue beitragen --- Schilddr\"usenstatus und Behandlung parallel pr\"ufen; eine sonst indizierte oder dringliche Depressionsbehandlung nicht allein bis zum Testergebnis verz\"ogern. \\
Symptomverlauf: Antriebslosigkeit seit November 2025, progredient; R\"uckenschmerzen seit 2024, fluktuierend.
\end{description}

\noindent Diese Vignette illustriert den vorgeschlagenen Ablauf, nicht klinische Validit\"at. Der beispielhafte DCI von 78\% besitzt keine kalibrierte klinische Interpretation und demonstriert nur die Berechnung. Der CAVE-Hinweis markiert einen m\"oglichen Schilddr\"usenbeitrag zur parallelen fachlichen Pr\"ufung; der Zeitpunkt einer Behandlung bleibt eine individualisierte klinische Entscheidung.
...
\item[Verantwortung und Produkthaftung] Verantwortung kann sich auf Behandelnde, Betreiber, Einrichtungen, Anbieter und Entwickler verteilen. Open-Source-Lizenz und Gew\"ahrleistungsausschl\"usse bestimmen oder beseitigen gesetzliche Haftung nicht automatisch. Pflichten, Fehler, Schaden und Kausalit\"at sind rechtsraumspezifisch zu pr\"ufen.
...
\textbf{Prozedur:} Phase~2 ist eine Machbarkeitsstudie zur Sch\"atzung der Reliabilit\"at, keine initiale Systemvalidierung. Die Rater bearbeiten zwei gematchte, nicht \"uberlappende Vignettens\"atze, je einen pro Methode; die S\"atze werden zwischen randomisierten Reihenfolgen gekreuzt. Perioden- und Sequenzeffekte werden ausgewiesen. Das Expertenpanel liefert eine verblindete Konsensreferenz mit dokumentierter Uneinigkeit und Unsicherheit, keinen Goldstandard.
...
\begin{itemize}[leftmargin=*, noitemsep]
    ...
    \item \textit{\"Ubereinstimmung mit der Konsensreferenz:} Deskriptive \"Ubereinstimmung und exakte Konfidenzintervalle. Diagnosespezifische Sensitivit\"at und Spezifit\"at erfordern eine sp\"atere, separat gepowerte Stichprobe.
    \item \textit{Exploratives DCI-Signal:} Verblindete Beurteilung handlungsrelevanter Informationsl\"ucken und Fehlalarmrate. Der Pilot testet keine konfirmatorische Patientennutzenhypothese.
...
\textbf{Stichprobenstatus:} $N=10$ Rater und 20 Vignetten bilden eine Machbarkeitsstichprobe f\"ur Ablauf, Rekrutierung, Missingness, Varianzkomponenten und Intervallsch\"atzung. Vor Rekrutierung sind ein prim\"arer Estimand und Analysemodell zu pr\"aregistrieren und die Zielstichprobe per Pr\"azisionsrechnung oder Simulation festzulegen. Der Pilot ist nicht f\"ur diagnosespezifische Genauigkeit, Patientennutzen oder \"Uberlegenheitsclaims gepowert.
...
\textbf{HL7 FHIR (R4):} Die folgenden Zuordnungen sind Kandidaten f\"ur einen k\"unftigen Implementation Guide, kein Interoperabilit\"atsnachweis. Diagnosen k\"onnten \nolinkurl{Condition}, Fragebogenantworten \nolinkurl{QuestionnaireResponse}, skalierte Funktions- und Assessmentergebnisse \nolinkurl{Observation}, die klinische Synthese gegebenenfalls \nolinkurl{ClinicalImpression}, Behandlungsplanung \nolinkurl{CarePlan} und Quelldokumente \nolinkurl{DocumentReference} verwenden. Die endg\"ultige Profilwahl erfordert Workflow-, Terminologie-, Provenienz-, Einwilligungs-, Rechtsraum- und Konformit\"atspr\"ufung.
...
Das vorgeschlagene 6-Achsen-System ist derzeit ein evaluierbarer Designvorschlag f\"ur strukturierte biopsychosoziale Dokumentation. Ob die symmetrische Architektur interprofessionelle Arbeit verbessert, der DCI n\"utzliche Informationsl\"ucken erkennt oder CAVE-Hinweise Sicherheit erh\"ohen, bleibt empirisch zu pr\"ufen. PRO/CONTRA-Eintr\"age und unkalibrierte Konfidenzwerte sind Nachvollziehbarkeitshilfen, keine Evidenz f\"ur diagnostische Genauigkeit oder klinischen Nutzen.
